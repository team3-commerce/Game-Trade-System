...
public class MemberItemInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        Item armor = itemRepository.findByName("갑옷").orElseThrow();
        /**
         * 인벤토리 생성, 획득 시간은 생성시간으로 설정했음
         */
        memberItemRepository.save(MemberItem.create(member, sword, LocalDateTime.now(), 10L));
        memberItemRepository.save(MemberItem.create(member, armor, LocalDateTime.now(), 5L));
    }
}
